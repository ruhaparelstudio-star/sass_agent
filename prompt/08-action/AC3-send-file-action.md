...
Create validated send_file/pricelist action with tenant asset ownership check.

## Scope

- Execute allowed actions only
- Blocked action must never dispatch
...
- Blocked action skipped
- Action log stored
- Tenant ownership validated

## Exit Criteria

Action can run safely through dispatcher.
